...
<div class="hdf">
	...
	<div>
		...
		<div style="margin-top: 6px;">
		    <?php if(is_array($up)): $i = 0; $__LIST__ = $up;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$up): $mod = ($i % 2 );++$i;?>上一篇： <a href="__URL__/eContent/id/<?php echo ($up["id"]); ?>" title="<?php echo ($up["title"]); ?>">
			    <?php echo (utf8substr($up["title"],0,32)); ?>
			</a>  <br><?php endforeach; endif; else: echo "" ;endif; ?>
		    <?php if(is_array($next)): $i = 0; $__LIST__ = $next;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$next): $mod = ($i % 2 );++$i;?>下一篇： <a href="__URL__/eContent/id/<?php echo ($next["id"]); ?>" title="<?php echo ($next["title"]); ?>">
		...
		<div class="rank">
		    <?php if(is_array($rank)): $i = 0; $__LIST__ = $rank;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rank): $mod = ($i % 2 );++$i;?><span> 
			    <a href="__URL__/eContent/id/<?php echo ($rank["id"]); ?>" title="<?php echo ($rank["title"]); ?>">
	...
	<!--   上传      -->
	<div class="up_fen">
	    <h2 style="text-align: center;">分享你的云盘电子书下载</h2>
	    <br>
	    <form action="__URL__/upFenE" method="post" name="form" >
