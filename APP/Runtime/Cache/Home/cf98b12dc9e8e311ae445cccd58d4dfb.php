...
<div class="hdf">
	...
	<div>
		...
		<div style="margin-top: 6px;">
		    <?php if(is_array($up)): $i = 0; $__LIST__ = $up;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$up): $mod = ($i % 2 );++$i;?>上一篇： <a href="__URL__/tesContent/id/<?php echo ($up["id"]); ?>" title="<?php echo ($up["title"]); ?>">
			    <?php echo (utf8substr($up["title"],0,32)); ?>
			</a>  <br><?php endforeach; endif; else: echo "" ;endif; ?>
		    <?php if(is_array($next)): $i = 0; $__LIST__ = $next;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$next): $mod = ($i % 2 );++$i;?>下一篇： <a href="__URL__/tesContent/id/<?php echo ($next["id"]); ?>" title="<?php echo ($next["title"]); ?>">
		...
		<div class="rank">
		    <?php if(is_array($rank)): $i = 0; $__LIST__ = $rank;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rank): $mod = ($i % 2 );++$i;?><span> 
			    <a href="__URL__/tesContent/id/<?php echo ($rank["id"]); ?>" title="<?php echo ($rank["title"]); ?>">
	...
	<!--   上传      -->
	<div class="up_fen">
	    <h2 style="text-align: center;">分享你的云盘试题下载</h2>
	    <br>
	    <form action="__URL__/upFenT" method="post" name="form" >
		<table style="clear: both;float: left;">
			...
			<td>简介：</td>
			<td>
			    <textarea id="kindeditor_id" name="content"  style="width: 560px;height: 300px;text-align: left;"></textarea>
			</td>
		    </tr>
		    <tr>
			<td>链接地址：</td>
			<td><input type="text" name="yun" class="fen_inp"></td>
		    </tr>
		    <tr>
			<td>&nbsp;</td>
